...
const CourseClassroom: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pt-28 pb-32 px-6 bg-slate-50 selection">
      {/* Background Ambience */}
      ...
      <div className="max-w-[1400px] mx-auto relative z-10">
        ...
        <header className="mb-12">
          <Link to="/dashboard" className="inline-flex items-center gap-2 text-slate-500 hover transition-colors text-sm font-bold mb-6 group">
          ...
          <div className="flex flex-col lg:flex-row lg justify-between gap-8">
            <div className="max-w-3xl">
              ...
              <h1 className="text-4xl md font-black text-slate-900 tracking-tighter leading-none mb-6">
                {course.title}
              </h1>
            </div>
            ...
            {role === 'teacher' && (
              <div className="flex gap-3">
                <button onClick={()=>setShowResourceModal(true)} className="px-6 py-4 bg-white border border-slate-200 rounded-2xl font-black text-xs uppercase tracking-widest hover transition-all shadow-sm">
                  Add Resource
                </button>
                <button onClick={()=>setShowModuleModal(true)} className="px-6 py-4 bg-purple-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover transition-all shadow-xl shadow-purple-600/20 flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 lg gap-10">
          {/* Main Content Area */}
          <div className="lg space-y-12">
            ...
            <div className="flex items-center gap-4 p-2 bg-white rounded-3xl border border-slate-200 w-fit">
              <button 
                ...
                className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-bold text-sm transition-all ${activeTab === 'roadmap' ? 'bg-purple-600 text-white shadow-lg shadow-purple-600/20' : 'text-slate-500 hover'}`}
              >
                <Layout className="w-4 h-4" /> Curriculum Roadmap
              </button>
              <button 
                onClick={() => setActiveTab('chat')}
                className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-bold text-sm transition-all ${activeTab === 'chat' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'text-slate-500 hover'}`}
              >
            ...
            {modules.length === 0 ? (
              <div className="py-24 text-center bg-white/50 backdrop-blur rounded-[3rem] border-2 border-dashed border-slate-200">
                ...
                {modules.map((mod, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`group bg-white backdrop-blur-xl border-2 rounded-[2.5rem] transition-all duration-500 overflow-hidden ${
                        isCompleted ? 'border-emerald-500/20 shadow-emerald-500/5' : 'border-slate-200/50 shadow-xl'
                      } ${isExpanded ? 'shadow-2xl border-purple-500/30' : 'hover:-translate-y-1 hover'}`}>
                        
                        <div className="p-8 md cursor-pointer" onClick={() => toggleModule(mod.id)}>
                          <div className="flex flex-col md:flex-row gap-8 items-start">
                            <div className={`w-20 h-20 md md rounded-3xl overflow-hidden flex-shrink-0 bg-slate-100 flex items-center justify-center ${isCompleted ? 'ring-4 ring-emerald-500/20' : ''}`}>
                              {mod.thumbnailUrl ? (
                                <img src={mod.thumbnailUrl} className="w-full h-full object-cover group-hover transition-transform duration-700" alt="" />
                              ) : (
                            ...
                            <div className="flex-1">
                              <div className="flex justify-between items-start mb-4">
                                <h3 className="text-2xl font-black text-slate-900 tracking-tight group-hover transition-colors">
                                  {mod.title}
                                </h3>
                                {isCompleted && (
                            ...
                            <div className={`mt-4 md:mt-0 p-3 rounded-2xl bg-slate-50 transition-transform duration-300 ${isExpanded ? 'rotate-90' : ''}`}>
                            ...
                            >
                              <div className="px-8 pb-10 md:px-10 space-y-6">
                                  ...
                                  {(!mod.lectures || mod.lectures.length === 0) ? (
                                    ...
                                    <div className="grid grid-cols-1 md gap-4">
                                      {mod.lectures.map((lec, lIdx) => (
                                        <div key={lec.id} className="p-5 bg-slate-50/50 rounded-3xl border border-transparent hover hover transition-all group/lec">
                                          <div className="flex justify-between items-start mb-4">
                                            ...
                                            <div className="flex gap-2">
                                              {lec.meetingLink && (
                                                <a href={lec.meetingLink} target="_blank" rel="noreferrer" className="p-2 bg-blue-500/10 text-blue-500 rounded-lg hover hover transition-all">
                                                  <Video className="w-4 h-4" />
                                                </a>
                                              )}
                                              {lec.recordedLink && (
                                                <a href={lec.recordedLink} target="_blank" rel="noreferrer" className="p-2 bg-rose-500/10 text-rose-500 rounded-lg hover hover transition-all">
                                                  <PlayCircle className="w-4 h-4" />
                                                </a>
                                              )}
                                            </div>
                                          </div>
                                          <h4 className="font-bold text-slate-900 mb-1 group-hover/lec transition-colors">
                                            {lec.title}
                                          </h4>
                                ...
                                <div className="flex items-center justify-between pt-8 border-t border-slate-100">
                                  {role === 'teacher' && (
                                    <button onClick={() => setShowLectureModal(mod.id)} className="flex items-center gap-2 px-6 py-3 bg-purple-500/10 text-purple-600 rounded-2xl font-black text-xs uppercase tracking-widest hover hover transition-all">
                                  ...
                                  {role === 'student' && (
                                    <button 
                                      ...
                                      className={`ml-auto flex items-center gap-2 px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${
                                        ...
                                        : 'bg-slate-900 text-white hover active'
                                      }`}
                                    >
          ...
          <aside className="lg space-y-8 order-first lg:order-last">
            {/* Progress Card */}
            <div className="lg:sticky lg:top-32 space-y-8">
              ...
              {/* Resources Card */}
              <div className="bg-white backdrop-blur-xl p-8 rounded-[2.5rem] border border-slate-200 shadow-xl">
                  ...
                  <div className="space-y-3">
                    {resources.map((res, i) => (
                      <motion.a 
                        ...
                        target="_blank" 
                        rel="noreferrer" 
                        className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-transparent hover hover transition-all group"
                      >
                        <div className="w-10 h-10 rounded-xl bg-purple-500/10 text-purple-500 flex items-center justify-center group-hover transition-transform">
                          <LinkIcon className="w-4 h-4" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="font-bold text-sm text-slate-800 truncate group-hover transition-colors">{res.title}</p>
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">External Asset</p>
                        </div>
                        <ExternalLink className="w-4 h-4 text-slate-300 opacity-0 group-hover transition-all" />
                      </motion.a>
                    ))}
                  </div>
                )}
              ...
              <div className="p-10">
                <div className="flex justify-between items-start mb-8">
                  ...
                  <button 
                    ...
                    className="p-3 bg-slate-100 rounded-2xl hover transition-colors"
                  >
                    <X className="w-6 h-6" />
                  </button>
                </div>
                  ...
                  <form onSubmit={handleCreateModule} className="space-y-6">
                    <div className="space-y-4">
                      <div>
                        ...
                        <input required placeholder="E.g. Foundational Theory" value={mTitle} onChange={e=>setMTitle(e.target.value)} className="w-full p-4 bg-slate-50 rounded-2xl outline-none font-bold border border-transparent focus transition-all" />
                      </div>
                      <div>
                        <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Mission Description</label>
                        <textarea required placeholder="What's the core objective?" value={mDesc} onChange={e=>setMDesc(e.target.value)} className="w-full p-4 bg-slate-50 rounded-2xl outline-none font-medium border border-transparent focus transition-all resize-none" rows={3} />
                      </div>
                    ...
                    <button type="submit" className="w-full py-5 bg-purple-600 text-white font-black rounded-[2rem] shadow-xl shadow-purple-600/20 hover transition-all">
                      DEPLOY MODULE
                    </button>
                  </form>
                )}
                ...
                {showLectureModal && (
                  <form onSubmit={handleAddLecture} className="space-y-6">
                    <div className="space-y-4">
                      <input required placeholder="Session Title" value={lTitle} onChange={e=>setLTitle(e.target.value)} className="w-full p-4 bg-slate-50 rounded-2xl outline-none font-bold border border-transparent focus transition-all" />
                      <input placeholder="Live Meeting Link (Optional)" type="url" value={lMeet} onChange={e=>setLMeet(e.target.value)} className="w-full p-4 bg-slate-50 rounded-2xl outline-none font-bold border border-transparent focus transition-all" />
                      <input placeholder="Recording Link (Optional)" type="url" value={lRec} onChange={e=>setLRec(e.target.value)} className="w-full p-4 bg-slate-50 rounded-2xl outline-none font-bold border border-transparent focus transition-all" />
                    </div>
                    <button type="submit" className="w-full py-5 bg-indigo-600 text-white font-black rounded-[2rem] shadow-xl shadow-indigo-600/20 hover transition-all">
                      SYNC SESSION
                    </button>
                  </form>
                )}
                ...
                {showResourceModal && (
                  <form onSubmit={handleCreateResource} className="space-y-6">
                    <div className="space-y-4">
                      <input required placeholder="Asset Title" value={rTitle} onChange={e=>setRTitle(e.target.value)} className="w-full p-4 bg-slate-50 rounded-2xl outline-none font-bold border border-transparent focus transition-all" />
                      <input required placeholder="Direct URL (Drive/Dropbox)" type="url" value={rUrl} onChange={e=>setRUrl(e.target.value)} className="w-full p-4 bg-slate-50 rounded-2xl outline-none font-bold border border-transparent focus transition-all" />
                    </div>
                    <button type="submit" className="w-full py-5 bg-emerald-600 text-white font-black rounded-[2rem] shadow-xl shadow-emerald-600/20 hover transition-all">
                      UPLOAD ASSET
                    </button>
                  </form>
                )}
