...
const Profile: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pt-32 pb-32 px-6 bg-slate-50 relative overflow-hidden">
      <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-gradient-to-br from-emerald-500/5 to-indigo-500/5 rounded-full blur-[120px] pointer-events-none -translate-y-1/2 translate-x-1/3" />
      <motion.div
      ...
      >
        <div className="flex items-center justify-between mb-10">
           <div className="flex items-center gap-4">
               <Link to="/dashboard" className="p-2.5 text-slate-500 hover bg-white/80 backdrop-blur border border-slate-200/50 rounded-2xl transition-colors shadow-sm">
           ...
           {!isEditing && (
              <button onClick={() => setIsEditing(true)} className="flex items-center gap-2 bg-slate-100 hover text-slate-800 font-semibold py-2 px-4 rounded-xl transition-colors">
                 <Edit3 className="w-4 h-4" /> Edit Profile
              </button>
           )}
        </div>

        <div className="bg-white/90 backdrop-blur-2xl rounded-[2.5rem] p-10 border border-slate-200/50 shadow-2xl">
           
           <div className="flex flex-col md:flex-row items-center gap-8 mb-10 pb-10 border-b border-slate-100">
             <div className="relative group">
                <div className="w-32 h-32 rounded-full border-4 border-slate-100 flex items-center justify-center overflow-hidden bg-emerald-50">
                   {selectedFile ? (
                ...
                {isEditing && (
                   <label className="absolute bottom-0 right-0 p-3 bg-emerald-600 hover text-white rounded-full cursor-pointer shadow-lg transition-transform transform group-hover">
             ...
             <div className="text-center md flex-1">
                <h2 className="text-3xl font-black text-slate-900 capitalize mb-1 tracking-tight">{userProfile.name}</h2>
           ...
           <div className="space-y-6">
              <div className="grid grid-cols-1 md gap-8">
                 <div>
                   ...
                   {isEditing ? (
                      <input type="text" value={editClass} onChange={(e) => {setEditClass(e.target.value); setSuccessMsg('');}} placeholder="e.g. B.Tech / High School" className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus focus outline-none text-slate-900 transition-all" />
                   ) : (
                      <p className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-slate-800 font-medium">
                 ...
                 <div>
                   ...
                   {isEditing ? (
                      <input type="text" value={editLocation} onChange={(e) => {setEditLocation(e.target.value); setSuccessMsg('');}} placeholder="e.g. Bangalore" className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus focus outline-none text-slate-900 transition-all" />
                   ) : (
                      <p className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-slate-800 font-medium">
              ...
              <div>
                ...
                {isEditing ? (
                   <input type="text" value={editInterests} onChange={(e) => {setEditInterests(e.target.value); setSuccessMsg('');}} placeholder="e.g. Math, Coding, Art (comma-separated)" className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus focus outline-none text-slate-900 transition-all" />
                ) : (
                   <div className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl min-h-[56px] flex flex-wrap gap-2">
              ...
              <div className="pt-10 mt-10 border-t border-slate-100">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button 
                    ...
                    className="text-sm font-bold text-emerald-600 hover transition-colors"
                  >
                  ...
                  <form onSubmit={handleChangePassword} className="space-y-4 animate-in slide-in-from-top-2 duration-300">
                    <div>
                      ...
                      <input 
                        ...
                        className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus focus transition-all font-medium"
                      />
                    </div>
                    ...
                    <button 
                      ...
                      className="w-full py-4 bg-slate-900 text-white font-bold rounded-2xl shadow-lg hover transition-all disabled flex items-center justify-center gap-2"
                    >
              ...
              <div className="pt-6 flex flex-col sm:flex-row items-center justify-end gap-4 border-t border-slate-100">
                 {isEditing ? (
                    <>
                       <button onClick={() => { setIsEditing(false); setSuccessMsg(''); }} disabled={saving} className="w-full sm hover text-slate-700 font-bold py-4 px-8 rounded-2xl transition-all">
                          Cancel
                       </button>
                       <button onClick={handleSaveProfile} disabled={saving} className="w-full sm bg-emerald-600 hover text-white font-bold py-4 px-8 rounded-2xl transition-all shadow-lg hover flex items-center justify-center gap-2 active disabled">
                          {saving ? <Loader2 className="w-5 h-5 animate-spin" /> : <Save className="w-5 h-5" />}
                          Save Changes
                       </button>
                    </>
                 ) : (
                    <button onClick={handleLogout} className="w-full sm bg-red-50 hover text-red-600 font-bold py-4 px-8 rounded-2xl transition-all flex items-center justify-center gap-2 active">
