...
const ADMIN_EMAILS = ['viranadeep@gmail.com', 'umakrishnakanthchokkapu15@gmail.com'];
...
const AdminDashboard: React.FC = () => {
  ...
  const handleDeleteCourse = (id: string, title: string) => {
    toast((t) => (
      <div className="flex flex-col gap-4 p-4 bg-white rounded-2xl">
        ...
        <p className="text-slate-600">Are you sure you want to permanently delete <span className="font-bold text-slate-900">"{title}"</span>? This action cannot be undone.</p>
        <div className="flex gap-2 justify-end mt-2">
          <button className="px-4 py-2 rounded-xl text-sm font-bold bg-slate-100 text-slate-600 hover transition-colors" onClick={() => toast.dismiss(t.id)}>Cancel</button>
          <button className="px-4 py-2 rounded-xl text-sm font-bold bg-red-500 hover text-white transition-all shadow-lg shadow-red-500/20" onClick={async () => {
            toast.dismiss(t.id);
            try {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 selection">
      {/* Mobile Sidebar Toggle */}
      <button 
        onClick={() => setIsSidebarOpen(true)}
        className="md:hidden fixed top-6 left-6 z-[60] p-3 bg-white border border-slate-200 rounded-xl shadow-lg"
      >
        ...
        {(isSidebarOpen || window.innerWidth >= 768) && (
          <>
            ...
            <motion.nav 
              ...
              className="fixed left-0 top-0 h-full w-72 md bg-white border-r border-slate-200/50 z-[60] flex flex-col p-8 shadow-2xl md"
            >
              <div className="mb-12 flex items-center justify-between">
                ...
                <button onClick={() => setIsSidebarOpen(false)} className="md:hidden p-2 hover rounded-lg transition-colors">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="flex-1 space-y-3">
                {[
                  ...
                  >
                    ...
                    <item.icon className={`w-6 h-6 ${activeTab === item.id ? 'text-white' : 'group-hover transition-transform'}`} />
              ...
              <div className="mt-auto pt-8 border-t border-slate-100">
                <button 
                  onClick={() => navigate('/')}
                  className="w-full flex items-center gap-4 px-5 py-4 rounded-2xl font-bold text-rose-500 hover transition-all border border-transparent hover"
                >
      ...
      <main className="md:pl-72 pt-32 md:pt-12 pb-24 px-6 md:px-16">
        <div className="max-w-[1400px] mx-auto">
          {/* Header & Search */}
          <header className="flex flex-col lg:flex-row lg justify-between gap-8 mb-16">
            ...
            <div className="relative group w-full lg">
              <div className="absolute inset-0 bg-emerald-500/20 rounded-2xl blur-xl group-focus-within transition-all duration-500 opacity-0 group-focus-within"></div>
              <div className="relative flex items-center">
                <Search className="absolute left-5 w-5 h-5 text-slate-400 group-focus-within transition-colors" />
                <input 
                  type="text" 
                  placeholder="Global database query..." 
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="w-full pl-14 pr-6 py-4 bg-white border border-slate-200 rounded-2xl focus:outline-none focus focus focus transition-all font-bold"
                />
                <div className="absolute right-4 px-2 py-1 bg-slate-100 rounded text-[10px] font-black text-slate-400 border border-slate-200 pointer-events-none">
                  ⌘ K
                </div>
              </div>
            </div>
            ...
            >
              {activeTab === 'courses' && (
                <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 lg gap-8">
                    <div className="lg bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
                      ...
                      <form onSubmit={handleCreateCourse} className="grid grid-cols-1 md gap-6">
                        <div className="col-span-1 md">
                          ...
                          <input required value={newCourse.title} onChange={e=>setNewCourse({...newCourse, title: e.target.value})} className="w-full p-4 bg-slate-50 rounded-2xl border border-transparent focus focus transition-all outline-none font-bold" placeholder="E.g. Advanced Quantum Computing" />
                        </div>
                        <div className="col-span-1 md">
                          <label className="block text-xs font-black uppercase tracking-widest text-slate-400 mb-2">Overview</label>
                          <textarea required value={newCourse.description} onChange={e=>setNewCourse({...newCourse, description: e.target.value})} className="w-full p-4 bg-slate-50 rounded-2xl border border-transparent focus focus transition-all outline-none font-medium resize-none" rows={4} placeholder="Describe the learning outcome..." />
                        </div>
                        <div>
                          <label className="block text-xs font-black uppercase tracking-widest text-slate-400 mb-2">Category</label>
                          <select value={newCourse.category} onChange={e=>setNewCourse({...newCourse, category: e.target.value})} className="w-full p-4 bg-slate-50 rounded-2xl border border-transparent focus transition-all outline-none font-bold appearance-none">
                        ...
                        <div>
                          ...
                          <input type="number" required value={newCourse.price} onChange={e=>setNewCourse({...newCourse, price: e.target.value as any})} className="w-full p-4 bg-slate-50 rounded-2xl border border-transparent focus transition-all outline-none font-bold" />
                        </div>
                        <div className="col-span-1 md">
                          <label className="block text-xs font-black uppercase tracking-widest text-slate-400 mb-2">Media Assets</label>
                          <div className="flex flex-col md:flex-row gap-4">
                            <label className="flex-1 flex items-center justify-center gap-3 p-4 bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl cursor-pointer hover transition-colors">
                            ...
                            <input type="url" value={newCourse.thumbnailUrl} onChange={e=>setNewCourse({...newCourse, thumbnailUrl: e.target.value})} placeholder="Or paste image URL..." className="flex-1 p-4 bg-slate-50 rounded-2xl border border-transparent focus transition-all outline-none text-sm font-medium" />
                          </div>
                        </div>
                        <div className="col-span-1 md">
                          <button type="submit" disabled={creatingCourse} className="w-full py-4 bg-emerald-500 hover text-white font-black rounded-2xl shadow-lg shadow-emerald-500/20 transition-all disabled flex items-center justify-center gap-2">
                    ...
                    <div className="grid grid-cols-1 md gap-4">
                      {filteredCourses.map(c => (
                        <motion.div 
                          layout
                          key={c.id} 
                          className="group bg-white p-6 rounded-[2rem] border border-slate-200 hover transition-all duration-300"
                        >
                          {editingCourseId === c.id ? (
                            <form onSubmit={(e) => { e.preventDefault(); handleUpdateCourse(); }} className="space-y-4">
                              <input value={editCourseData.title} onChange={e=>setEditCourseData({...editCourseData, title: e.target.value})} className="w-full p-3 rounded-xl bg-slate-50 font-bold" required />
                              <textarea value={editCourseData.description} onChange={e=>setEditCourseData({...editCourseData, description: e.target.value})} className="w-full p-3 rounded-xl bg-slate-50 font-medium" required rows={3} />
                              <div className="flex gap-2">
                                <button type="submit" className="flex-1 bg-emerald-500 text-white p-3 rounded-xl font-bold">Save Changes</button>
                                <button type="button" onClick={()=>setEditingCourseId(null)} className="p-3 bg-slate-100 rounded-xl font-bold">Cancel</button>
                              </div>
                            </form>
                          ) : (
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-start mb-1">
                                  ...
                                  <div className="flex gap-1">
                                    <button onClick={() => { setEditingCourseId(c.id); setEditCourseData(c); }} className="p-2 text-slate-400 hover hover rounded-xl transition-all">
                                      <Edit className="w-5 h-5"/>
                                    </button>
                                    <button onClick={() => handleDeleteCourse(c.id, c.title)} className="p-2 text-slate-400 hover hover rounded-xl transition-all">
                                ...
                                <div className="flex items-center gap-3">
                                  <span className="text-xs font-black uppercase tracking-wider px-3 py-1 bg-slate-100 rounded-full text-slate-500">
                                    {c.category}
                                  </span>
              ...
              {activeTab === 'users' && (
                <div className="bg-white rounded-[2.5rem] border border-slate-200 overflow-hidden shadow-2xl">
                  <div className="p-10 border-b border-slate-100 flex flex-col md:flex-row md justify-between gap-6">
                    <div>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex -space-x-3">
                        {usersList.slice(0, 5).map((u, i) => (
                          <div key={i} className="w-10 h-10 rounded-full bg-slate-100 border-2 border-white flex items-center justify-center text-[10px] font-black text-slate-500">
                            {(u.name || 'U').charAt(0)}
                          </div>
                        ))}
                      </div>
                      <span className="px-4 py-2 bg-emerald-500/10 text-emerald-600 rounded-xl text-[10px] font-black uppercase tracking-widest border border-emerald-500/20">
                    ...
                    <table className="w-full text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {filteredUsers.map((usr) => (
                          <tr key={usr.uid} className="hover transition-colors group">
                            <td className="px-10 py-6">
                              <div className="flex items-center gap-5">
                                <div className="relative">
                                  <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center font-black text-slate-500 text-lg group-hover transition-transform">
                                ...
                                <div>
                                  <div className="font-black text-slate-900 flex items-center gap-2">
                                    ...
                                    {usr.email && ADMIN_EMAILS.includes(usr.email) && (
                                      <span className="px-3 py-1 bg-gradient-to-r from-emerald-500/20 to-teal-500/20 text-emerald-400 text-[10px] font-black uppercase tracking-widest rounded-full border border-emerald-500/20 flex items-center gap-1">
                            ...
                            <td className="px-10 py-6 text-right">
                              {(usr.email && !ADMIN_EMAILS.includes(usr.email)) && (
                                <button className="p-3 text-slate-300 hover hover rounded-2xl transition-all group/btn">
                                  <Trash2 className="w-5 h-5 group-hover/btn transition-transform" />
                                </button>
                              )}
                            </td>
                          </tr>
              ...
              {activeTab === 'appointments' && (
                <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 md lg gap-8">
                    {teacherApps.length === 0 ? (
                      ...
                      teacherApps.map(app => (
                        <motion.div 
                          layout
                          key={app.id} 
                          className="group bg-white p-8 rounded-[2.5rem] border border-slate-200/50 hover,0,0,0.1)],0,0,0.3)] transition-all duration-500 relative overflow-hidden"
                        >
                          ...
                          <button 
                            ...
                            className="w-full py-4 bg-slate-900 text-white font-black rounded-2xl hover active transition-all shadow-xl group-hover group-hover"
                          >
                            REVIEW DOSSIER
                          </button>
                        </motion.div>
              ...
              {activeTab === 'patchnotes' && (
                <div className="max-w-3xl mx-auto space-y-8">
                  <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200">
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <input placeholder="Version (e.g. 1.2.0)" className="p-4 bg-slate-50 rounded-2xl outline-none font-bold" />
                        <input placeholder="Update Title" className="p-4 bg-slate-50 rounded-2xl outline-none font-bold" />
                      </div>
                      <textarea placeholder="Describe the changes..." className="w-full p-4 bg-slate-50 rounded-2xl outline-none font-medium resize-none" rows={4} />
                      <button className="w-full py-4 bg-slate-900 text-white font-black rounded-2xl">
                        PUBLISH PATCH NOTES
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-6">
                    {patchNotes.map(note => (
                      <div key={note.id} className="p-6 bg-white rounded-[2rem] border border-slate-200">
                 ...
                 <div className="bg-white p-12 rounded-[3.5rem] border border-slate-200 shadow-3xl relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                      <div className="flex items-center gap-5 mb-12">
                         ...
                         <div>
                            <h3 className="text-3xl font-black tracking-tight text-slate-900">Admin Subsystems</h3>
                            <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em]">Critical Infrastructure Control</p>
                         </div>
                      </div>

                      <div className="space-y-8">
                        <section className="p-8 border-2 border-dashed border-rose-500/20 rounded-[2.5rem] bg-rose-500/5 group hover transition-colors">
                           <div className="flex items-start gap-4 mb-6">
                             ...
                             <div>
                                ...
                                <p className="text-slate-500 font-medium text-sm leading-relaxed">This operation protocols a complete erasure of all Firestore nodes (Curricula, Users, Logs). Access keys and Auth profiles will persist, but all relational data will be nullified.</p>
                             </div>
                           </div>
                           ...
                           <div className="flex items-center gap-4">
                             <button 
                               onClick={wipeAllData}
                               className="px-8 py-5 bg-rose-600 text-white font-black rounded-2xl shadow-2xl shadow-rose-600/30 hover active transition-all flex items-center gap-3 uppercase tracking-widest text-xs"
                             >
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <button className="p-6 bg-slate-50 rounded-3xl border border-slate-100 text-left group">
                            <span className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Backups</span>
                            <span className="text-lg font-bold text-slate-900 group-hover transition-colors">Snapshot State</span>
                          </button>
                          <button className="p-6 bg-slate-50 rounded-3xl border border-slate-100 text-left group">
                            <span className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Logs</span>
                            <span className="text-lg font-bold text-slate-900 group-hover transition-colors">Audit Console</span>
                          </button>
                        </div>
                      </div>
                    </div>
        ...
        {selectedApp && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-2xl bg-white rounded-[3rem] border border-slate-200 shadow-2xl overflow-hidden"
            >
              <div className="p-10">
                <div className="flex justify-between items-start mb-10">
                  ...
                  <button onClick={() => setSelectedApp(null)} className="p-3 bg-slate-100 rounded-2xl hover transition-colors">
                    <X className="w-6 h-6" />
                  </button>
                </div>
                    ...
                    <div className="pt-8 border-t border-slate-100">
                        ...
                        <div className="bg-slate-50 p-6 rounded-[2rem] mb-6">
                          ...
                          <div className="flex gap-4">
                            <input 
                              ...
                              className="flex-1 p-4 bg-white rounded-2xl outline-none font-bold border border-transparent focus transition-all" 
                            />
                            <button 
                              onClick={() => handleApproveApp(selectedApp.id, selectedApp.userEmail)}
                              className="px-8 bg-emerald-500 text-white font-black rounded-2xl hover transition-all"
                            >
                              SEND
                            </button>
                          </div>
                      ...
                      <div className="flex gap-4">
                        <button 
                          ...
                          className="flex-1 py-5 bg-emerald-500 text-white font-black rounded-[2rem] shadow-xl shadow-emerald-500/20 hover transition-all"
                        >
                          APPROVE MENTOR NOW
                        </button>
                        <button 
                          onClick={() => handleFinalVerdictTeacher(selectedApp.id, selectedApp.userEmail, 'rejected')}
                          className="flex-1 py-5 bg-red-500 text-white font-black rounded-[2rem] shadow-xl shadow-red-500/20 hover transition-all"
                        >
                          REJECT APPLICATION
                        </button>
                      </div>
