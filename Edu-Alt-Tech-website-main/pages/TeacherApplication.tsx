...
const TeacherApplication: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pt-32 pb-32 px-6 bg-slate-50 relative overflow-hidden">
      <div className="absolute top-0 left-1/2 w-[700px] h-[700px] -translate-x-1/2 -translate-y-1/2 bg-gradient-to-br from-emerald-500/5 to-indigo-500/5 rounded-full blur-[120px] pointer-events-none" />
      <motion.div
      ...
      >
        <button onClick={() => navigate(-1)} className="inline-flex items-center gap-2 text-slate-500 hover transition-colors mb-8 font-medium">
          <ArrowLeft className="w-4 h-4" /> Back
        </button>

        <div className="bg-white/90 backdrop-blur-2xl p-8 md rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-slate-200/50">
          <h1 className="text-4xl font-black text-slate-900 mb-3 tracking-tight">Apply to Teach</h1>
          <p className="text-slate-600 mb-10 pb-10 border-b border-slate-100 font-medium">
            You are applying to teach: <strong className="text-slate-900">{course.title}</strong>
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                <p className="font-semibold text-slate-900">{user?.displayName || 'Teacher'}</p>
                <p className="text-sm text-slate-500">{user?.email}</p>
              </div>
            </div>

            <div>
              <label className="block text-sm font-bold text-slate-700 mb-2">Preferred Appointment Scenario</label>
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full pl-12 pr-4 py-3 bg-white rounded-xl border border-slate-200 focus:outline-none focus focus shadow-sm"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <input 
                ...
                placeholder="e.g. 5"
                className="w-full p-4 bg-white rounded-xl border border-slate-200 focus:outline-none focus focus shadow-sm"
              />
            </div>

            <div>
              <label className="block text-sm font-bold text-slate-700 mb-2">Talents & Skills</label>
              <textarea 
                rows={2}
                required
                value={skills}
                ...
                className="w-full p-4 bg-white rounded-xl border border-slate-200 focus:outline-none focus focus shadow-sm resize-none"
              ></textarea>
            </div>

            <div>
              <label className="block text-sm font-bold text-slate-700 mb-2">Proposed Curriculum Path</label>
            ...
            <button 
              ...
              className="w-full py-4 bg-emerald-600 hover text-white font-bold rounded-xl transition-all shadow-lg flex items-center justify-center gap-2"
            >
