  ...
  return (
    <div className="min-h-screen pt-32 pb-32 px-6 bg-slate-50 relative overflow-hidden">
      ...
      <motion.div
        ...
        ref={contentRef}
      >
        <Link to="/courses" className="inline-flex items-center gap-2 text-slate-500 hover transition-colors mb-10 font-medium">
          <ArrowLeft className="w-4 h-4" /> Back to Courses
        </Link>

        {/* Hero Card */}
        <div className="bg-white/90 backdrop-blur-2xl rounded-[2.5rem] p-10 md border border-slate-200/50 shadow-2xl mb-8">
          <div className="flex flex-col md:flex-row justify-between gap-6 mb-8">
            <div>
              <div className={`mb-4 w-fit px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${course.category === 'education' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'}`}>
                {course.category}
              </div>
              <h1 className="text-4xl md font-black text-slate-900 mb-4 tracking-tight leading-[1.05]">{course.title}</h1>
              <p className="text-xl text-slate-600 font-medium">{course.description}</p>
            </div>
          ...
          <div className="bg-slate-50 p-6 md rounded-3xl border border-slate-100">
            {enrollment ? (
                ...
                {enrollment.role === 'student' ? (
                   enrollment.paymentStatus === 'pending' ? (
                     <>
                        ...
                        <h2 className="text-2xl font-bold text-slate-900 mb-2">Mentor Assigned!</h2>
                        <p className="text-slate-600 max-w-md mx-auto mb-6">
                          A mentor is ready to teach you. Please complete the payment to start learning.
                        </p>
                        <button className="bg-emerald-600 hover text-white font-bold py-4 px-8 rounded-xl shadow-lg transition-all w-full md">
                  ...
                  <>
                     ...
                     <h2 className="text-2xl font-bold text-slate-900 mb-2">Teacher Application Submitted</h2>
                     <p className="text-slate-600 max-w-md mx-auto mb-6">
              ...
              <div className="flex flex-col gap-6">
                    ...
                    <div className="space-y-4">
                      {mentors.map(m => (
                        <div 
                           ...
                           className={`p-4 border-2 rounded-xl cursor-pointer transition-all ${selectedMentor === m.userId ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200 hover'}`}
                        >
                      ...
                      <button 
                        ...
                        className="w-full bg-slate-900 text-white font-bold py-4 px-8 rounded-xl hover transition-all shadow-md disabled mt-4 flex justify-center items-center gap-2"
                      >
                ...
                <div className="flex-1 bg-slate-50/80 backdrop-blur border border-purple-200/50 p-6 rounded-2xl flex flex-col justify-center items-center text-center shadow-sm">
                  ...
                  {myAppStatus === 'pending' || myAppStatus === 'scheduled' ? (
                     ...
                     <button disabled className="w-full max-w-xs bg-slate-200 text-rose-500 font-bold py-3 px-8 rounded-xl cursor-not-allowed border border-rose-200">
                     ...
                     <button 
                       ...
                       className="w-full max-w-xs bg-white text-purple-600 border-2 border-purple-200 hover font-bold py-3 px-8 rounded-xl transition-all shadow-sm"
                     >
                       Apply to Teach
                     </button>
                  )}
