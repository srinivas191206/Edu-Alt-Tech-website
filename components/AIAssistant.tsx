                    ...
                    <div className="absolute top-full left-0 mt-1 bg-white rounded-xl border border-slate-200 shadow-xl p-1 z-10 min-w-[200px]">
                      {MODES.map(m => (
                        <button
                          ...
                          className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs font-bold transition-colors ${mode === m.id ? 'bg-emerald-50 /20 text-emerald-600 ' : 'text-slate-600 hover:bg-slate-50 :bg-slate-700'}`}
                        >
                          {m.icon}
