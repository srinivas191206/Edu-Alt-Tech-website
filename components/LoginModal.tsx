      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden z-10 p-8 md:p-10 max-h-[90vh] overflow-y-auto custom-scrollbar"
          >
            {/* Close Button */}
            <button
              onClick={() => { onClose(); resetState(); }}
              className="absolute top-6 right-6 p-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-500 dark:text-slate-400 rounded-xl transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="text-center mb-8 pr-8">
              ...
              <h2 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white mb-2 tracking-tight">{title}</h2>
              <p className="text-sm text-slate-500 dark:text-slate-400 font-semibold">{subtitle}</p>
            </div>

            {error && (
              <div className="mb-6 p-4 bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-xl border border-red-100 dark:border-red-800 text-xs font-bold">
                {error}
              </div>
            )}
            ...
            <form onSubmit={mode === 'login' ? handleLogin : handleSignup} className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-12 pr-4 py-3 bg-slate-50 dark:bg-slate-800 dark:text-white dark:placeholder-slate-500 rounded-xl border border-slate-200 dark:border-slate-700 focus:border-emerald-500 outline-none transition-colors"
                      />
                    </div>
                  </div>
                  <div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="••••••••"
                    className="w-full pl-12 pr-12 py-3 bg-slate-50 dark:bg-slate-800 dark:text-white dark:placeholder-slate-500 rounded-xl border border-slate-200 dark:border-slate-700 focus:border-emerald-500 outline-none transition-colors"
                  />
                  <button
                    type="button"
              ...
              <button
                ...
                className="w-full py-3.5 bg-slate-900 dark:bg-emerald-600 text-white font-bold rounded-xl hover:bg-slate-800 dark:hover:bg-emerald-500 transition-colors shadow-lg flex items-center justify-center gap-2"
              >
            ...
            <button
              ...
              disabled={loading || googleLoading}
              className="w-full py-3.5 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 font-bold rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 hover:border-slate-300 transition-colors shadow-sm flex items-center justify-center gap-3"
            >
              {googleLoading ? (
                ...
                {mode === 'login' ? (
                  <>
                    ...
                    <button
                      ...
                      className="font-bold text-slate-900 dark:text-emerald-400 hover:text-emerald-600 transition-colors underline underline-offset-4"
                    >
                      Sign Up
                    </button>
                  </>
