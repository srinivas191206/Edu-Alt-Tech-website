...
function ProblemCard({ problem, user, onLockedClick }: { problem: LeetCodeProblem; user: any; onLockedClick: () => void }) {
 ...
 animate={{ opacity: 1, y: 0 }}
  className="bg-surface border border-border rounded-xl p-3 sm:p-4 hover:shadow-lg hover:border-emerald-500 transition-all duration-300 min-w-0"
  >
  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 sm:gap-3">
  <div className="flex-1 min-w-0">
   <div className="flex items-center gap-2 mb-1">
                   <span className="text-[10px] sm:text-xs font-bold text-text-muted shrink-0">#{problem.num}</span>
                   <h3 className="text-sm sm:text-base font-bold text-heading truncate break-words">{problem.title}</h3>
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-[11px] sm:text-xs font-medium text-text-secondary break-words">{problem.topic}</span>
  ...
  <span key={i} className="text-[10px] font-semibold text-text-muted bg-surface-2 px-1.5 py-0.5 rounded truncate max-w-[120px] sm:max-w-none">
  {c.name} {c.count > 0 && `(${c.count})`}
  </span>
  ))}
  {problem.companies.length > 3 && (
  <span className="text-[10px] text-text-muted shrink-0">+{problem.companies.length - 3}</span>
  )}
  </div>
  )}
 </div>
  <div className="flex items-center gap-2 w-full sm:w-auto shrink-0 justify-end mt-3 sm:mt-0 pt-3 sm:pt-0 border-t border-border/50 sm:border-t-0">
  <a
...
function CourseCard({ course, user, onLockedClick }: { course: CourseLink; user: any; onLockedClick: () => void }) {
  ...
  return (
    <a href={course.url} target="_blank" rel="noopener noreferrer" onClick={handleAction}
      className="block bg-surface border border-border rounded-xl p-4 hover:shadow-lg hover:border-emerald-500 transition-all duration-300 group min-w-0"
    >
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="font-bold text-heading text-sm group-hover:text-emerald-600 transition-colors whitespace-normal break-words">{course.title}</h3>
        </div>
        <ExternalLink className="w-4 h-4 text-text-muted shrink-0 group-hover:text-emerald-500 transition-colors" />
      </div>
    </a>
  );
}
  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex-1 min-w-0">
          <h3 className="font-bold text-heading text-sm group-hover:text-emerald-600 transition-colors whitespace-normal break-words">{interview.company}</h3>
          <p className="text-xs text-text-secondary mt-0.5 whitespace-normal break-words">{interview.interviewType}</p>
        </div>
        <div className="flex items-center justify-between sm:justify-end gap-3 pt-2.5 sm:pt-0 border-t border-border/50 sm:border-t-0 shrink-0">
...
function EnglishExerciseCard({ exercise, user, onLockedClick }: { exercise: EnglishExercise; user: any; onLockedClick: () => void }) {
 ...
 <motion.div layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
  ...
  <div className="flex-1 min-w-0">
  <div className="flex items-center gap-2 mb-1">
                   <span className="text-[10px] sm:text-xs font-bold text-text-muted shrink-0">#{exercise.num}</span>
                   <h3 className="text-sm sm:text-base font-bold text-heading truncate break-words">{exercise.title}</h3>
 </div>
 <div className="flex items-center gap-2 flex-wrap">
  <span className="px-2 py-0.5 rounded-md text-[10px] font-bold border bg-indigo-100 text-indigo-700 border-indigo-200 ">
 {exercise.level}
 </span>
 </div>
 </div>
 <div className="flex items-center gap-2 sm:gap-1 w-full sm:w-auto shrink-0 justify-end mt-3 sm:mt-0 pt-3 sm:pt-0 border-t border-border/50 sm:border-t-0">
...
function ChannelCard({ channel, user, onLockedClick }: { channel: YouTubeChannel; user: any; onLockedClick: () => void }) {
  ...
  return (
    <a href={channel.url} target="_blank" rel="noopener noreferrer" onClick={handleAction}
      className="flex items-center gap-3 bg-surface border border-border rounded-xl p-4 hover:shadow-lg hover:border-emerald-500 transition-all duration-300 group min-w-0"
    >
      <div className="w-10 h-10 rounded-xl bg-red-100/20 text-red-500 flex items-center justify-center shrink-0">
        <Youtube className="w-5 h-5" />
      </div>
      <div className="flex-1 min-w-0">
        <h3 className="font-bold text-heading text-sm group-hover:text-emerald-600 transition-colors whitespace-normal break-words">{channel.name}</h3>
        <p className="text-xs text-text-secondary mt-0.5 whitespace-normal break-words">{channel.category}</p>
      </div>
      <ExternalLink className="w-4 h-4 text-text-muted shrink-0 group-hover:text-emerald-500 transition-colors" />
    </a>
  );
...
const Practice: React.FC = () => {
 ...
 return (
  <div className="practice-page-container min-h-screen pt-24 pb-20 sm:pt-32 sm:pb-32 px-4 md:px-6 bg-surface relative overflow-hidden">
  ...
  <h1 className="text-3xl sm:text-5xl md:text-7xl font-black text-heading mb-6 tracking-tighter leading-[0.9]">
  ...
  <p className="text-sm sm:text-lg text-text-secondary max-w-xl font-medium">
  460+ LeetCode problems, 52 English grammar topics, video solutions, full courses, and interview prep.
  </p>
  </motion.div>

 {/* Tabs */}
 <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="flex gap-2 mb-8 border-b border-border pb-4 overflow-x-auto no-scrollbar">
 {tabs.map(t => (
 ...
 {t.icon} {t.label}
  ...
  problemSet === 'popular' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'bg-surface-2 text-text-secondary hover:bg-surface-2 border border-border '
  ...
  problemSet === 'leetcode150' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'bg-surface-2 text-text-secondary hover:bg-surface-2 border border-border '
  ...
  problemSet === 'top150' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'bg-surface-2 text-text-secondary hover:bg-surface-2 border border-border '
   ...
   problemSet === 'admin' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'bg-surface-2 text-text-secondary hover:bg-surface-2 border border-border '
  ...
  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="flex flex-col sm:flex-row gap-3 mb-8">
    ...
    <Search className="absolute left-2.5 sm:left-3 top-1/2 -translate-y-1/2 w-3.5 sm:w-4 h-3.5 sm:h-4 text-text-muted" />
    <input type="text" placeholder="Search problems..." value={search} onChange={e => setSearch(e.target.value)}
    className="w-full pl-8 sm:pl-10 pr-2.5 sm:pr-3 py-2.5 bg-surface border border-border rounded-xl text-xs sm:text-sm focus:ring-2 focus:ring-emerald-500 outline-none text-heading placeholder-text-muted"
    />
    </div>
  <div className="flex gap-2 w-full sm:w-auto">
    <select value={topicFilter} onChange={e => setTopicFilter(e.target.value)}
    className="flex-1 sm:flex-none px-2.5 sm:px-3 py-2.5 bg-surface border border-border rounded-xl text-xs sm:text-sm font-medium text-text-secondary focus:ring-2 focus:ring-emerald-500 outline-none min-w-0 sm:min-w-[140px] truncate"
    >
  ...
  {(search || topicFilter || diffFilter) && (
    <button onClick={() => { setSearch(''); setTopicFilter(''); setDiffFilter(''); }}
      className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-2.5 bg-surface border border-border rounded-xl text-[10px] sm:text-xs font-bold text-text-secondary hover:text-red-500 hover:border-red-200 transition-colors shrink-0">
 ...
 <p className="text-center text-text-muted py-12 font-medium">No problems match your filters.</p>
 )}
 </>
 )}
  ...
  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="flex flex-wrap gap-3 sm:gap-4 mb-6">
    <div className="relative flex-1 min-w-[140px] sm:min-w-[200px] max-w-sm">
      ...
      <input type="text" placeholder="Search courses..." value={search} onChange={e => setSearch(e.target.value)}
        className="w-full pl-8 sm:pl-10 pr-2 sm:pr-3 py-2 sm:py-2.5 bg-surface border border-border rounded-xl text-xs sm:text-sm focus:ring-2 focus:ring-emerald-500 outline-none text-heading placeholder-text-muted"
      />
    </div>
  </motion.div>
 ...
 <>
  ...
  />
   ...
   className="px-2.5 sm:px-3 py-2 sm:py-2.5 bg-surface border border-border rounded-xl text-xs sm:text-sm font-medium text-text-secondary focus:ring-2 focus:ring-emerald-500 outline-none min-w-[100px] max-w-[140px] sm:max-w-none sm:min-w-[140px] truncate"
  >
  ...
  {(englishSearch || levelFilter) && (
    <button onClick={() => { setEnglishSearch(''); setLevelFilter(''); }}
      className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-2 sm:py-2.5 bg-surface border border-border rounded-xl text-[10px] sm:text-xs font-bold text-text-secondary hover:text-red-500 hover:border-red-200 transition-colors">
      <X className="w-3 sm:w-3.5 h-3 sm:h-3.5" /> Clear
    </button>
  )}
  </motion.div>
 ...
 <p className="text-center text-text-muted py-12 font-medium">No English exercises match your criteria.</p>
 )}
 </>
 )}
 ...
 className="relative mt-12 py-8 sm:py-16 px-4 sm:px-8 rounded-3xl bg-surface/20 border border-border/50 backdrop-blur-2xl text-center overflow-hidden shadow-2xl">
...
<h2 className="text-2xl sm:text-3xl font-black text-heading mb-4 tracking-tight">
