...
const Login: React.FC = () => {
 ...
 transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }}
  className="w-full max-w-md bg-surface/90 backdrop-blur-2xl p-10 md:p-12 rounded-2xl shadow-2xl shadow-border/50 border border-border/50 relative z-10"
  >
   <div className="text-center mb-10">
   <img src="/logo.png" alt="EduAltTech Logo" loading="lazy" className="w-14 h-14 mx-auto mb-4 object-contain" />
   <h1 className="text-4xl font-black text-heading mb-3 tracking-tight">Welcome Back</h1>
  <p className="text-text-secondary font-medium">Continue your execution journey.</p>
 </div>
  ...
  className="w-full pl-14 pr-5 py-4 bg-bg rounded-xl border border-border focus:border-emerald-400 focus:ring-4 focus:ring-emerald-100 outline-none transition-all"
  />
  </div>
  </div>
  <div>
  ...
  placeholder="••••••••"
  className="w-full pl-14 pr-12 py-4 bg-bg rounded-xl border border-border focus:border-emerald-400 focus:ring-4 focus:ring-emerald-100 outline-none transition-all"
  />
  <button
  type="button"
  onClick={() => setShowPassword(!showPassword)}
  className="absolute right-5 top-1/2 -translate-y-1/2 text-text-muted hover:text-text-secondary transition-colors"
  >
  ...
  <span className="bg-surface px-4 text-text-muted font-bold tracking-widest">Or continue with</span>
  </div>
  </div>
  ...
  Don't have an account? <Link to="/signup" className="font-bold text-heading hover:text-primary transition-colors underline underline-offset-4">Sign Up</Link>
  </p>
  </div>
  </form>
 </motion.div>
