...
const BehaviorInsights: React.FC = () => {
 ...
 return (
 <div className="min-h-screen pt-28 pb-32 px-6 bg-bg selection:bg-emerald-500/30">
 ...
 <button onClick={() => navigate(-1)} className="inline-flex items-center gap-2 text-text-secondary hover:text-heading transition-colors text-sm font-bold mb-8 group">
 ...
 <p className="text-text-secondary font-medium">Student engagement patterns and dropout risk detection</p>
 </div>
 </div>

 <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
 <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
 className="bg-surface p-6 rounded-2xl border border-border shadow-xl">
 <Users className="w-8 h-8 text-emerald-500 mb-3" />
 <p className="text-3xl font-black mb-1">{students.length}</p>
 <p className="text-xs font-bold text-text-muted uppercase tracking-widest">Total Students</p>
 </motion.div>
 <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
 className="bg-surface p-6 rounded-2xl border border-border shadow-xl">
 <AlertTriangle className="w-8 h-8 text-rose-500 mb-3" />
 <p className="text-3xl font-black mb-1">{atRiskCount}</p>
 <p className="text-xs font-bold text-text-muted uppercase tracking-widest">At Risk</p>
 </motion.div>
 <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
 className="bg-surface p-6 rounded-2xl border border-border shadow-xl">
 <Target className="w-8 h-8 text-emerald-500 mb-3" />
 <p className="text-3xl font-black mb-1">{stableCount}</p>
 <p className="text-xs font-bold text-text-muted uppercase tracking-widest">On Track</p>
 </motion.div>
 </div>
 ...
 sortBy === s.key ? 'bg-amber-500 text-white shadow-lg' : 'bg-surface text-text-secondary border border-border '
 }`}
 ...
 <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-text-muted">Student</th>
 <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-text-muted">Status</th>
 <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-text-muted">Engagement</th>
 <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-text-muted">Activities</th>
 <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-text-muted">Avg Score</th>
 <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-text-muted">Risk</th>
 </tr>
 </thead>
 ...
 return (
 <tr key={s.uid} className="hover:bg-bg transition-colors">
 <td className="px-8 py-5">
 <div className="flex items-center gap-3">
 <span className="w-8 h-8 rounded-xl bg-surface-2 flex items-center justify-center text-xs font-black">{i + 1}</span>
 <div>
 <p className="font-bold text-sm">{s.name}</p>
 <p className="text-[10px] text-text-muted">{s.email}</p>
 </div>
 </div>
 </td>
 <td className="px-8 py-5">
 ...
 <span className="text-xs font-bold text-text-secondary">{s.activityCount > 10 ? 'Active' : s.activityCount > 3 ? 'Intermittent' : 'Inactive'}</span>
 </div>
 </td>
 <td className="px-8 py-5">
 <div className="flex items-center gap-2">
